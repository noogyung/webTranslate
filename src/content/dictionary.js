...
\n  /* ── 사전 조회 대상 유효성 검증 ──────────────────────────── */
  ...
  document.addEventListener("mouseup", async (e) => {
    ...
    setTimeout(async () => {
      ...
      var settings = cachedSettings || (await sendToBackground({ action: "getSettings" }));
      if (!settings) return;
  ...
  async function executePopupTranslation(selectedText, cleanWord, rect, settings, forceMode) {
    ...
    if (forceMode === "dict") {
      ...
      if (dictCache.has(cacheKey)) {
        var cached = dictCache.get(cacheKey);
  ...
  function attachToggleButton() {
    ...
    var btn = activeDictPopup.querySelector(".wt-mode-toggle");
    if (btn) {
      btn.addEventListener("mousedown", (e) => {
        e.stopPropagation();
        e.preventDefault();
        var nextMode = currentPopupContext.mode === "dict" ? "sentence" : "dict";
