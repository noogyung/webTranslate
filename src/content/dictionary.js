...
/* ────────────────────────────────────────────
  ...
  document.addEventListener("mouseup", async (e) => {
    ...
    setTimeout(async () => {
      ...
      // 이미 사용자가 설정한 목표 언어로 되어 있는 경우 팝업 즉시 스킵
      if (isAlreadyTargetLang(cleanWord, settings.targetLang)) return;

      var range = selection.getRangeAt(0);
      var rect = range.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) return;

      executePopupTranslation(selectedText, cleanWord, rect, settings);
    }, 150);
  });

  state.currentPopupContext = null;

  export async function executePopupTranslation(selectedText, cleanWord, rect, settings) {
    state.currentPopupContext = { selectedText, cleanWord, rect, settings };
    removeDictPopup();
  ...
  export function renderDictPopup(data, rect) {
    if (!state.activeDictPopup) return;
    var { word, pronunciation, definitions } = data;

    var html = `
      <div class="wt-dict-header" style="justify-content:space-between; align-items:flex-start;">
    ...
    if (Array.isArray(definitions) && definitions.length > 0) {
      ...
      posMap.forEach((items, pos) => {
        ...
        items.forEach((def, idx) => {
          ...
          var prefix = items.length === 1 ? "" : `${idx + 1}. `;
          html += `<span class="wt-dict-meaning">${prefix}${meaningText}</span>`;

          // 대표 예문
          if (def.example && (def.example.source || def.example.en)) {
            var sourceEx = def.example.source || def.example.en;
            var targetEx = def.example.target || def.example.ko || (typeof def.example === "object" ? Object.values(def.example)[1] : "");
            html += `
              <div class="wt-dict-example">
                <div class="wt-dict-example-en">${sourceEx}</div>
                ${targetEx ? `<div class="wt-dict-example-ko">${targetEx}</div>` : ""}
              </div>
            `;
          }
          html += `</div>`;
