...
import { applyLocalDictionary, buildDictRegex } from "./translation.js";
...
/* ────────────────────────────────────────────
   ...
   */
  export function normalizeDateTimes(text) {
    if (!text || typeof text !== "string") return text;
    return text.replace(/(\b\d{4})(\d{1,2}:\d{2})\b/g, "$1 $2");
  }

  export function needsRemoteTranslation(text, customDict) {
    var temp = text;
  ...
  export function shouldTranslateText(text, targetLang, element = null, isSelection = false) {
    ...
    var dict = state.cachedSettings ? state.cachedSettings.customDict : [];
  ...
  export function checkIsPureText(element) {
    return !element.querySelector(state.COMPLEX_CHILD_SEL);
  }

  export function isVisuallyHidden(element) {
    if (!element) return false;
    if (element.hidden || element.getAttribute("aria-hidden") === "true") return true;
    if (element.closest?.(state.HIDDEN_ANCESTOR_SEL)) return true;
  ...
  export function isSliderDot(element, text) {
    if (!element) return false;
