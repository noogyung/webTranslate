...
* **이미지 번역 진단**: 옵션의 **이미지 번역 진단 열기**에서 원본 이미지를 선택하고 **OCR + 왕복 검사**를 실행합니다. OCR 좌표, 실제 크롭, 전송 스프라이트와 GPT 없는 복원 결과를 확인한 뒤 별도 버튼으로 AI 합성을 실행할 수 있습니다. **진단 묶음 저장**은 이미지·원문·번역·좌표를 JSON으로 저장하며, 같은 페이지에서 다시 열 수 있습니다. API 키는 저장하지 않습니다. 실제 AI 합성은 API 비용이 발생하며 진단 호출은 일반 사용 횟수 통계에 포함되지 않습니다.
